...
@section('title')
<title> {{__('nav.newRequest')}} | IMS</title>
@endsection
                                ...
                                <table id="departmentTb" class="display table table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($new as $item)
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    @if (($checkRole->role_name == 'admin' || $checkRole->role_name == 'Admin') || $action->action_edit == 1) 
                                                       | <a type="button" data-bs-toggle="modal" data-bs-target="#recoveryCategory{{$item->borrow_id}}" ><span class="mdi mdi-email-check-outline fs-5 text-success" title="Accept request"></span></a> |
                                                        <a type="button" data-bs-toggle="modal" data-bs-target="#rejected{{$item->borrow_id}}" ><span class="mdi mdi-email-remove-outline fs-5 text-danger" title="Reject request"></span></a>
                                                    @endif
                                            ...
                                            {{-- reject modal / --}}
                                            <div class="modal fade" id="rejected{{$item->borrow_id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="rejected{{$item->borrow_id}}Label" aria-hidden="true">
                                                
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="rejected{{$item->borrow_id}}"> {{__('nav.rejectRequest')}} </h1>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
                                                        <form action="{{route('request.reject')}}" method="post" autocomplete="off">

                                                            <div class="modal-body">
                                                                @csrf
                                                                <input type="hidden" class="d-none" name="borrowId" value="{{$item->borrow_id}}">
                                                                <input type="hidden" class="d-none" name="proId" value="{{$item->pro_id}}">
                                                                <input type="hidden" class="d-none" name="borrowQty" value="{{$item->borrow_qty}}">
                                                                <label for="" class="text-danger"> {{__('nav.rejectQuestion')}} <span class="text-primary fw-bold">{{$item->name_en}}</span> ?</label>
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="submit" class="ui button red">
                                                                    <span class="mdi mdi-email-remove-outline"></span> {{__('nav.reject')}}
                                                                </button>
                                                            </div>
                                                        </form>
                                            ...
                                            {{-- <div class="modal fade" id="deleteVerify{{$item->borrow_id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="deleteVerify{{$item->borrow_id}}Label" aria-hidden="true">
                                             ...
                                             <!-- Modal recovery-->
                                            <div class="modal fade" id="recoveryCategory{{$item->borrow_id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="recoveryCategory{{$item->borrow_id}}Label" aria-hidden="true">
                                                
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="recoveryCategory{{$item->borrow_id}}">{{__('nav.acceptRequest')}}</h1>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
                                                        <form action="{{route('request.accept')}}" method="post" autocomplete="off">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <label for=""> {{__('nav.acceptQuestion')}} <span class="text-primary fw-bold">{{$item->name_en}}</span> ?</label>
                                                            </div>
                                                            <div class="modal-footer">
                                                                <button type="submit" class="ui button primary">
                                                                    <span class="mdi mdi-email-check-outline"></span> {{__('nav.yes')}}
                                                                </button>
                                                            </div>
                                                        </form>
