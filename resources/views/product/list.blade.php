...
@section('title')
    <title> {{ $instock ? __('nav.productInStock') : __('nav.productOutstock')}} | IMS</title>
@endsection
...
@section('content')
            ...
            <div class="col-md-12 grid-margin">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class="font-weight-bold">{{ $instock ? __('nav.productInStock') : __('nav.productOutstock')}}</h3>
                        <h6 class="font-weight-normal mb-0">{{ __('nav.product') }} / <span class="text-primary"><a
                                    class="text-primary" href="">{{ $instock ? __('nav.productInStock') : __('nav.productOutstock')}}</a></span></h6>
                    </div>
                </div>
                <div class="row mt-4">
                                    ...
                                    @if ($checkRole->role_name == 'admin' || $checkRole->role_name == 'Admin' || $action->action_edit == 1)
                                        <div class="col-8 col-sm-7 mb-2  col-md-7">
                                            <div class="ui small icon buttons">
                                                ...
                                                <a href="{{ $instock ? route('product.exportInstock') : route('product.exportOutstock') }}" class="ui button fw-normal"
                                                    title="Excel"><i class="file excel icon"></i>Excel</a>
                                                <a href="{{ $instock ? route('product.exportInstockPdf') : route('product.exportOutstockPdf') }}"
                                        ...
                                        @if ($instock)
                                            <div class="col-4 col-sm-5 mb-2  col-md-5 text-end">
                                                <a href="{{ route('product.form') }}" class="ui button primary tiny"><span
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($products as $key => $item)
                                                <tr>
                                                    <td style="width: 10px"
                                                        class="text-start
                                                    {{ $item->delete_status == '0' ? 'text-danger' : '' }} ">
                                                        {{ $key + 1 }}
                                                    </td>
                                                    ...
                                                    <td
                                                        class="text {{ $item->delete_status == '0' ? 'text-danger' : '' }}">
                                                        <p><span class="text-muted">Code :</span> {{ $item->item_code }}
                                                        </p>
                                                        <p>
                                                        ...
                                                        <p><span class="text-muted">Cate :</span> {{ $item->cat_name }}</p>
                                                        <p><span class="text-muted">Type :</span>
                                                            {{ $item->equipment_type == 1 ? __('nav.equipment') : __('nav.accessories') }}
                                                        </p>
                                                        <p class="text-break w-100 text-wrap"><span class="text-muted">Note :</span>
                                                    ...
                                                    <td class="{{ $item->delete_status == '0' ? 'text-danger' : '' }}">
                                                        @if ($item->qty == '0')
                                                            <div class="ui red label">
                                                                {{ __('nav.Outstock') }}
                                                                <div class="detail">x {{ $item->qty }}</div>
                                                            </div>
                                                        @else
                                                    ...
                                                    <td>
                                                        <div
                                                            class="ui icon top right pointing dropdown dropdown{{ $key + 1 }} button tiny circular p-2">
                                                            <span class="mdi mdi-dots-vertical"></span>
                                                            <div class="menu">
                                                                {{-- <div class="header">Display Density</div> --}}
                                                                <a href="{{ route('product.show', $item->id) }}"
                                                                    title="View detail" class="item">
                                                                    <span
                                                                        class="mdi mdi-eye-outline icon"></span>{{ __('nav.detail') }}
                                                                </a>

                                                                @if ($checkRole->role_name == 'admin' || $checkRole->role_name == 'Admin' || $action->action_edit == 1)
                                                                    <a title="Update FAC" class="item"
                                                                        id="updateFACButton" data-id="{{ $item->id }}">
                                                                        <span
                                                                            class="mdi mdi-pencil icon"></span>{{ __('nav.updateFAC') }}
                                                                    </a>
                                                                    <a class="item"
                                                                        href="{{ route('product.edit', $item->id) }}"
                                                                        title="Update data">
                                                                        <span
                                                                            class="mdi mdi-square-edit-outline icon"></span>
                                                                        {{ __('nav.update') }}
                                                                    </a>
                                                                @endif
                                                                ...
                                                                @if ($checkRole->role_name == 'admin' || $checkRole->role_name == 'Admin' || $action->action_delete == 1)
                                                                    @if ($item->delete_status == '0')
                                                                        <a type="button" id="restoreButton" class="item"
                                                                            data-id="{{ $item->id }}"
                                                                            title="Recovery data">
                                                                            <span
                                                                                class="mdi mdi-backup-restore icon"></span>
                                                                            {{ __('nav.restoreData') }}
                                                                        </a>
                                                                    @else
                                                                        <a type="button" title="Remove data"
                                                                            id="deleteButton" class="item"
                                                                            data-id="{{ $item->id }}">
                                                                            <span
                                                                                class="mdi mdi-trash-can-outline icon"></span>
                                                                            {{ __('nav.removeToTrash') }}
                                                                        </a>
                                                                    @endif
                                                                @endif
                                                            </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="updateForm{{ $item->id }}"><span class="mdi mdi-book-open"></span>
                        {{ __('nav.proDetail') }}</h1>
    ...
    <div class="modal fade" id="updateFacModal" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('product.updateFac', 1) }}" class="ui form" method="post" id="formUpdateFAC" autocomplete="off">
                    <div class="modal-body">
                        @csrf
                        {{-- <input type="hidden" class="d-none" name="proId" id="deleteProID" value=""> --}}
                        {{-- <h2 class="ui icon header w-100">
                            <div class="content mt-3">
                                <div class="sub header mt-2"> {{ __('nav.updateFAC') }} </div>
                            </div>
                        </h2> --}}
                        <div class="field">
                            <label for="fac">{{ __('nav.updateFAC') }} <span class="text-danger">*</span></label>
                            <input type="text" id="fac" name="fac" value="" autofocus placeholder="{{ __("nav.updateFAC") }}">
                        </div>
                    </div>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#updateFACButton', function() {
                var id = $(this).data('id');
                var urlUpdate = "{{ route('product.updateFac', ':id') }}".replace(':id', id);
                var urlGetData = "{{ route('product.editFac', ':id') }}".replace(':id', id);
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    success: function (response) {
                        $('#updateFacModal').modal('show');
                        $('#formUpdateFAC').attr('action', urlUpdate);
                        $('#fac').val(response.data.fix_asset_code);
                    }
                });
            });
        });
