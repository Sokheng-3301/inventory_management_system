...
@endif
@section('title')
<title> {{__('nav.editPurchaseRequest')}} | IMS</title>
@endsection
        ...
        <div class="col-md-8 grid-margin">
            <div class="row">
                <div class="col-12 mb-4 mb-xl-0">
                    <h3 class="font-weight-bold">{{__('nav.editPurchaseRequest')}}</h3>
                    <h6 class="font-weight-normal mb-0">{{__('nav.productPurchase')}} / <a class="text-primary" href="{{route('purchase.index')}}">{{__('nav.purchasing')}} </a> / 
                        <a class="text-primary" href="">{{__('nav.editPurchaseRequest')}} </a></h6>
                </div>
            </div>
            <div class="row mt-4">
                                        ...
                                        <form action="{{route('purchase.doedit', ['id' => $pr->id])}}" method="post" enctype="multipart/form-data" autocomplete="off" class="ui form">
                                            ...
                                            <select name="userAccount" id="userAccount" class="ui search dropdown d-block w-100 mb-2">
                                                ...
                                                @foreach ($users as $user)
                                                    <option {{ $user->id == $pr->requester ? 'selected' : '' }} value="{{$user->id}}">{{ $user->card_id .' - '. $user->name_kh . '  ' . $user->name_en}}</option>
                                                @endforeach
                                            </select>
                                                ...
                                                <div class="col-md-6">
                                                   ...
                                                   <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        {{-- <input type="number" name="price_unit" id="price_unit" min="0" value="{{ old('price_unit') }}" placeholder="Product price unit"> --}}
                                            ...
                                            <input type="file" name="attachment[]" multiple id="attachment" style="display: none;" class="d-none attachment" accept="image/*">
                                            ...
                                            <div id="preview" class="mt-3 border rounded-2 bg-white p-3">
                                                @php
                                                    $attExplode = explode(',', $pr->att);
                                                @endphp
                                                @foreach ($attExplode as $expAtt)
                                                    <img src="{{ asset($expAtt) }}" alt="Attachment file" width="45%">
                                                @endforeach
                                            </div>
