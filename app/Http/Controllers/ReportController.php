...
use App\Exports\AllExpenseReportExport;
...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        // echo $data['count_item'];
        // exit;
        // $data['item_counts'] = DB::table('products')->where('delete_status', 1)->count();
        return view('all-report.index', $data);
    }
    ...
    {
        $filename = 'expenses-report-all-' . now()->format('d-m-Y_h-i-s_a') . '.xlsx';
        return Excel::download(new AllExpenseReportExport(
            $request->year,
            $request->start_date,
            $request->end_date
        ), $filename);
    ...
    public function exportExpensePdf(Request $request) {
        ...
        $html = view('pdf.all-expense', ['expenses' => $data['expenses'], 'service_fees' => $data['service_fees'], 'title' => __("nav.serviceFee")])->render();
